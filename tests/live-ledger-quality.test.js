...
'use strict';
// What a real question gets from the live ledger: unrelated turns never fuse
// into one activity line, the same fact told five ways is one statement with
// its receipts, a possession question keeps no activity line, and a profane
// alias never reaches the cast.
...
t('activities on one generic entity stay apart unless their words agree', () => {
  ...
  ic.writeInstances({
    instances: [
      mk('Running a 100-question check with a local model', 0),
      mk('Working on memory optimization for the last six months', 1),
      mk('Signing the build before the release', 2),
      mk('Running the 100-question check with a local model again', 3)
    ],
  ...
  const bench = acts.find((p) => /100-question/.test(p.instance.description));
...
t('the same fact told five ways is one statement with its receipts', () => {
  ...
  const items = [
    { source: 'dialogue-window', id: 'a1', statement: 'User has a road bike', ts: asked - 5 * 86400000 },
    { source: 'dialogue-window', id: 'a2', statement: 'User owns a road bike', ts: asked - 4 * 86400000 },
    { source: 'dialogue-window', id: 'a3', statement: 'The user has a road bike.', ts: asked - 3 * 86400000 },
    { source: 'dialogue-window', id: 'a4', statement: 'user: there are 2 road bikes, the good one is the red one', ts: asked - 2 * 86400000 },
    { source: 'dialogue-window', id: 'a5', statement: 'The user owns or has access to a road bike.', ts: asked - 86400000 },
  ];
  const v = buildReconciledView(items, { noun_head: 'bikes', question: 'How many road bikes do I have?', reference_ts: asked });
  const out = v.render();
  ...
  assert.ok(sLines.some((l) => /there are 2 road bikes/.test(l)), 'a raw turn is never folded');
});
...
t('a question about what is owned never keeps an activity line, even when that empties the ledger', () => {
  ...
  const items = [
    inst('i1', 'activity', 'troth', 'Running a 100-question check with a local model', 'run'),
    inst('i2', 'activity', 'garden-plan', 'garden-plan project in the recent projects folder', 'have'),
    { source: 'dialogue-window', id: 'r9', statement: 'user: there are 2 road bikes, the good one is the red one', ts: asked - 86400000 },
  ];
  const v = buildReconciledView(items, { noun_head: 'bikes', head_phrase: 'road bikes', question: 'How many road bikes do I have?', reference_ts: asked });
  assert.ok(!v.ledger.some((l) => /run troth|100-question/.test(l.statement)), v.ledger.map((l) => l.statement).join(' | '));
  const run = v.aside.find((a) => /100-question/.test(a.item.statement));
...
t('an insult, a sentence or a clause is never an alias', () => {
  const w = identity.recordEntityIdentity({
    agent_id: 'claude-code', name: 'Contoso', kind: 'organization',
    aliases: ['contosso', 'i mana poutanes gioi tis contoso', 'the company that makes the tool and blocks the build every time', 'Contoso Ltd']
  });
  assert.ok(w && w.aliases, 'identity written');
  const al = w.aliases.map((a) => a.toLowerCase());
  assert.ok(al.includes('contosso') && al.includes('contoso ltd'), al.join(' | '));
