...
function _cachePaths() {
  ...
  // The partner's own work runs with the operator's environment (the open
  // partner ground), where mktemp reaches the OS temp directory on its own;
  // the walled contexts (an install, the workspace, confined ground by
  // choice) keep the user temp tree closed, or the confinement could not be
  // tested, since a test's own throwaway directories live there.
